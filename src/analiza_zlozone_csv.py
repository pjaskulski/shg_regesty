...
from zlozone import rule_patterns
...
if __name__ == "__main__":
    ...
    for nr in range(1, 13):
        ...
        output_path = Path('.').parent / f'output/zlozone_{slowniki[nr]}.txt'
        text_path = Path('.').parent / f'output/zlozone_text_{slowniki[nr]}.txt'
            ...
            for row in csv_reader:
                ...
                #        f_text.write(f'{p_full}\n\n')
                    ...
                    for span in spacy.util.filter_spans(spans):
                        ...
                        pattern = r'\s{1}[A-ZŚŻŹĆŁ]{1}\.\s{1}'
                        match = re.search(pattern, funkcja)
                        if match:
                            skrot_miejsc = match.group().strip()[0]
