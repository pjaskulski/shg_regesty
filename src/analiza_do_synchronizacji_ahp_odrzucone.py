...
def czy_miejscowosc(value_header: str, value_p0: str) -> tuple:
    ...
    if not result:
        przyczyna = "Hasło jest tylko odnośnikiem"
...
def select_from_db_by_id(sql):
    """ przetwarza zapytanie z wyszukiwaniem wg shg id """
    result = ''
    parafia = ''
    coordinate = ''
    lista_uwagi = []
    ...
    if sql_rows:
        if len(sql_rows) == 1:
            cur_row = sql_rows[0]
            result = field_strip(cur_row[0])
            parafia = field_strip(cur_row[3])
            coordinate = field_strip(cur_row[9])
        elif len(sql_rows) >= 1:
            for cur_row in sql_rows:
                if not result:
                    result = field_strip(cur_row[0])
                    parafia = field_strip(cur_row[3])
                    coordinate = field_strip(cur_row[9])
                else:
                    lista_uwagi.append(field_strip(cur_row[0]))


    return result, parafia, coordinate, ','.join(lista_uwagi)
...
if __name__ == '__main__':
    ...
    for nr in range(4, 5):
        ...
        output_path = Path('.').parent / f'shg_odrzucone/shg_odrzucone_{slowniki[nr]}_linki.xlsx'
        ...
        point_sheet_columns = ['SHG_ID', 'Nazwa', 'Odmianki', 'POWIAT', 'Powiaty',
                               'Parafie', 'AHP', 'AHP Parafia', 'Uwagi lub przyczyna odrzucenia',
                               'EXPERT']
        sheet_mejscowosci = create_sheet(wb=wb, title="Odrzucone", columns=point_sheet_columns)
        ...
        col = 4 # POWIAT
        dim_holder[get_column_letter(col)] = ColumnDimension(sheet_mejscowosci, min=col, max=col, width=12)
        col = 5 # Powiaty
        ...
        col = 8 # AHP Parafia
        dim_holder[get_column_letter(col)] = ColumnDimension(sheet_mejscowosci, min=col, max=col, width=15)
        col = 9 # Uwagi lub przyczyna odrzucenia
        dim_holder[get_column_letter(col)] = ColumnDimension(sheet_mejscowosci, min=col, max=col, width=30)
        col = 10 # EXPERT
        dim_holder[get_column_letter(col)] = ColumnDimension(sheet_mejscowosci, min=col, max=col, width=6)
        ...
        with open(input_path, mode='r', encoding='utf-8') as csv_file:
            ...
            for row in csv_reader:
                ...
                ok, przczyna_odrzucenia = czy_miejscowosc(nazwa, p0)
                if ok:
                    continue
                else:
                ...
                # czy jest w AHP"
                sql = f""" SELECT zbiorcza.id_miejscowosci, zbiorcza.nazwa_16w, zbiorcza.nazwa_odmianki,
                     zbiorcza.parafia, zbiorcza.powiat_p, zbiorcza.wojewodztwo,
                     zbiorcza.nazwa_niemiecka, zbiorcza.sgh_id, zbiorcza.woj_p,
                     ahp_pkt.WGS84 as coordinate
                    FROM zbiorcza INNER JOIN ahp_pkt ON zbiorcza.id_miejscowosci = ahp_pkt.id
                    WHERE
                    zbiorcza.sgh_id = '{miejsce_id}'
                """

                result, parafia, coordinate, dodatkowe = select_from_db_by_id(sql)
                expert = ''
                if result:
                    expert = 'TAK'

                akapit = tuple([miejsce_id, nazwa, '', '', '', '', result, parafia, przczyna_odrzucenia, expert])
        ...
        for row in sheet_mejscowosci.iter_rows(2, max_rows):
            m_id = row[0].value
            row[0].hyperlink = f'http://www.slownik.ihpan.edu.pl/search.php?id={m_id}'
            #row[3].value = f'http://www.slownik.ihpan.edu.pl/search.php?id={m_id}'
